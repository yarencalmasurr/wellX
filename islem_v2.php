...
try {
    ...
    elseif ($is == 'verileri_kaydet') {
        ...
        if ($mevcut) {
            $sorgu = $conn->prepare("UPDATE aktivite_kayitlari SET su_miktari=?, uyku_suresi=?, alinan_kalori=?, yakilan_kalori=?, spor_suresi=?, guncel_kilo=? WHERE id=?");
            $sorgu->execute([$su, $uyku, $alinan, $yakilan, $spor, $kilo, $mevcut['id']]);
            $mesaj = "guncellendi";
        } else {
            $sorgu = $conn->prepare("INSERT INTO aktivite_kayitlari (user_id, su_miktari, uyku_suresi, alinan_kalori, yakilan_kalori, spor_suresi, guncel_kilo, kayit_tarihi) VALUES (?,?,?,?,?,?,?,?)");
