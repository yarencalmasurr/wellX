...
<head>
    <meta charset="UTF-8">
    <title>Egzersizlerim | WellX</title>
    ...
    <style>
        ...
        .header-icon { background: linear-gradient(135deg, #3b82f6, #2563eb); width: 50px; height: 50px; border-radius: 16px; display: flex; align-items: center; justify-content: center; color: white; font-size: 24px; box-shadow: 0 8px 20px rgba(59,130,246,0.3);}
        ...
        .glass-card::before { content: ''; position: absolute; top: 0; left: 0; width: 6px; height: 100%; background: var(--blue); }
...
<div class="content">
    <div class="page-header">
        <div class="header-icon"><i class="fas fa-running"></i></div>
        <h1>Bugünün Antrenman Programları</h1>
    </div>
        ...
        <?php foreach ($planlar as $p): ?>
            <div class="glass-card">
                <div class="plan-header">
                    <h3><i class="fas fa-medal" style="color:var(--blue)"></i> Spor Hocası: <?php echo htmlspecialchars($p['hoca_adi']); ?></h3>
                    <span class="plan-date"><i class="far fa-calendar-alt"></i> <?php echo date('d.m.Y H:i', strtotime($p['kayit_tarihi'])); ?></span>
                </div>
                <div class="plan-text">
    ...
    <?php else: ?>
        <div class="no-plan">
            ...
            <p>Hocanız yeni bir program eklediğinde burada görünecektir. Bol bol dinlenin!</p>
        </div>
    <?php endif; ?>
</div>
